...
python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S01_poisoned_1.0
  --z-threshold 1.0
```bash
...
What does that tell you about whether the detector is working?the detector picked up one of the two poisoned cameras, so it's working but not sensitive enough 
What would you try next (different threshold, different scenario)? next I will run the clean and poisoned at a different threshold
```
