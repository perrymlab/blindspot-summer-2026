...
python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S03_poisoned
```bash
...
Were those the cameras the poison was injected into?no it was not
Did mean distance drop on the poisoned cameras compared to clean?yes for both of the poisoned cameras
What does that tell you about whether the detector is working?that it did not pick up the poisoned embeddings
What would you try next (different threshold, different scenario)?I would try a different threshold
```
