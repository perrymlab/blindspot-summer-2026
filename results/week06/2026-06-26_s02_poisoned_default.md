...
python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S02_poisoned_default
```bash
```
...
Were those the cameras the poison was injected into?it was one of the two that were poisoned
Did mean distance drop on the poisoned cameras compared to clean? yes for both poisoned cameras
What does that tell you about whether the detector is working?it is working but was not sensitive enough to pick up both poisoned cameras
What would you try next (different threshold, different scenario)?next I will try a different threshold
```
