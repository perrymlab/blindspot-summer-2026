...
python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S07_poisoned 
```bash
...
Were those the cameras the poison was injected into? it was one of the cameras that was poisoned
Did mean distance drop on the poisoned cameras compared to clean?yes
What does that tell you about whether the detector is working? The detector is picking up some poisoned embeddings but not all of them
What would you try next (different threshold, different scenario)?Try a different threshold
```
