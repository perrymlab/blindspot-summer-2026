...
## Command (Paste the exact command you ran, including all flags and the full input path)
 python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S07_poisoned_1.0 
  --z-threshold 1.0
```bash
...
Were those the cameras the poison was injected into?it was one of the two that were posisoned
Did mean distance drop on the poisoned cameras compared to clean?yes
What does that tell you about whether the detector is working?that it was sensitive enough to pick up the poisoned embeddings in c01 but not c02
