...
python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S05_poisoned 
```bash
...
Did mean distance drop on the poisoned cameras compared to clean?Yes it did for both poisoned cameras
What does that tell you about whether the detector is working?The detector did not flag one camera so it's potentially not sensitive enough
