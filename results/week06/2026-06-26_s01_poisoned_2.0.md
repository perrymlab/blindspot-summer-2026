...
python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S01_poisoned_2.0 
  --z-threshold 2.0
```bash
...
Were those the cameras the poison was injected into?it was one of two poisoned cameras
Did mean distance drop on the poisoned cameras compared to clean?yes for both poisoned cameras
What does that tell you about whether the detector is working?it seems like it is working but is not sensitive enough?
What would you try next (different threshold, different scenario)?next i will try a different scenario
```
