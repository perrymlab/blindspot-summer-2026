...
python scripts/analyze_embedding_export.py \
  ...
  --out-dir results/week06/S03_clean_1.0 
  --z-threshold 1.0
```bash
...
What would you try next (different threshold, different scenario)?next I will run the poisoned data to make comparisons
```

## Issues

```text
Anything that looked wrong or surprising:Camera 3 had a z-score of 33.7 which seems really high
