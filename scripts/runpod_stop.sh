...
# Network volume persists; runpod_resume.sh restores the rest on next start.
#
...
bash "$REPO_ROOT/scripts/prestop_check.sh"
rc=$?

if [ "$rc" -ne 0 ] && [ "$FORCE" -ne 1 ]; then
    echo
    echo "Pre-stop check raised warnings above. Re-run with --force to stop anyway."
    exit 1
fi
...
if command -v runpodctl >/dev/null 2>&1; then
    ...
    echo "No runpodctl and no RUNPOD_API_KEY -- stop the pod from the RunPod console."
    exit 1
fi
